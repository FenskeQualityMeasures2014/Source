function image = renderFocusLarge(lightfield, focalLength)
image = zeros(380,380,4);
%Lightfield is assumed to be of size [9,9,380,380,3];
lensletAngles = zeros(10,1);
...
for i=1:380
    for j=1:380
        ...
        disp([i j]);
        image(i,j,:) = lightfield(9+quadrant(1)*imageAngleIndex(abs(i-imageCenter(1))+1),9+quadrant(2)*imageAngleIndex(abs(j-imageCenter(2))+1),i,j,:);
    end
end
t = toc;
image = image(:,:,1:3);
