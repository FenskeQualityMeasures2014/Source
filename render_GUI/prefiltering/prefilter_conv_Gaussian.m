...
novel_filter_ratio = sign(novel_filter_ratio).*min(1,abs(novel_filter_ratio));
figure;
            ...
            for ii = 1:480
                for jj = 1:640
                    ...
                    if (pixel_depth > 0 && pixel_depth < 1)
                        ...
                        filtered_LF(i,j,ii,jj,rgb) = filterBank(filter_bank_index,:)*image2filter(ii:ii+24,jj:jj+24)*filterBank(filter_bank_index,:)';
                    else
...
for rgb = 1:3
            ...
            for ii = 1:480
                for jj = 1:640
                    ...
                    if (pixel_depth > 0 && pixel_depth < 1)
                        filter_bank_index = max(1,min(100,round(100*pixel_depth)));
                        filtered_novel(ii,jj,rgb) = filterBank(filter_bank_index,:)*image2filter(ii:ii+24,jj:jj+24)*filterBank(filter_bank_index,:)';
                    else
...
Filename = [Filename(1:end-4) '_filtered.mat'];
